...
describe("Course content edit mode", () => {
  ...
  it("does not submit activity lifecycle status when saving authoring fields", async () => { api.patch.mockResolvedValueOnce(response({ ...activity, title: "Updated watch" })); renderContent(); fireEvent.click(await screen.findByLabelText("Edit mode")); fireEvent.click(screen.getByLabelText("Actions for Watch")); fireEvent.click(screen.getByLabelText("Edit Watch")); fireEvent.change(screen.getByLabelText("Title", { exact: true }), { target: { value: "Updated watch" } }); fireEvent.click(screen.getByRole("button", { name: "Save changes" })); await waitFor(() => expect(api.patch).toHaveBeenCalledWith("/api/v1/admin/activities/activity-1", expect.objectContaining({ title: "Updated watch", config: {} }))); expect(api.patch.mock.calls[0][1]).not.toHaveProperty("status"); });
